...
app.controller('ListCtrl',
  function ($scope, $location, $route, Stripe, Google, Person, Service, POSTCODE_PATTERN, MOBILE_PATTERN) {
    ...
    $scope.register = function (code, result) {
      ...
      if (result.error) {
        if ($scope.error) { $scope.error.push(result.error.toString()); } else { $scope.error = [ result.error.toString() ]; }
      } else {
    ...
    $scope.addressZip = '';
    $scope.$watch('addressZip', function() {
      if(!$scope.registration.addressZip.$invalid) {
        $scope.addressCity = 'Plymouth';
        $scope.addressState = 'Devon';
        $scope.addressCountry = 'GB';
      } else {
        $scope.addressCity = '';
        $scope.addressState = '';
